...
def bubble_sort(arr1: np.array) -> np.array:
    sorted_array=np.copy(arr1)
    n=len(arr1)
    for a in range(n):
        for i in range(0,n-1):
            for j in range(0,n-i-1):
                if sorted_array[a][j]>sorted_array[a][j+1]:
                    sorted_array[a][j],sorted_array[a][j+1]=sorted_array[a][j+1],sorted_array[a][j]

    return sorted_array

@ray.remote
def bubble_sort_remote(arr1: np.array)-> np.array:
    return bubble_sort(arr1)
def run_local_sort(arr1:np.array):
    result=[bubble_sort(arr1) for _ in range(os.cpu_count())]
    return result
def run_remote_sort(arr1: np.array):
    result= ray.get([bubble_sort_remote.remote(arr1) for _ in range(os.cpu_count())])
    return result
print("Bubble sort:")
arr=np.copy(ray.get(obj_ref_arr1))
print(bubble_sort(arr))
print("sorted arr loc:")
cProfile.run("run_local_sort(ray.get(obj_ref_arr1))")
print("sorted arr rem:")
cProfile.run("run_remote_sort(ray.get(obj_ref_arr1))")
ray.shutdown()
