...
@ray.remote
def process_large_lists_and_dicts(l_list,l_dict):
    n= len(l_list)
    m= len(l_dict)
    sum_list=sum(l_list)
    sum_dict=sum(l_dict.values())
    sum_len=n+m
    sum_val=sum_list+sum_dict
    return sum_len,sum_val
...
list_ref=ray.put(large_list1)
dict_ref=ray.put(large_dict1)

def run_process_remote(l_list,l_dict):
    result = ray.get([process_large_lists_and_dicts.remote(l_list,l_dict) for _ in range(os.cpu_count())])
    return result

print("TASK")
cProfile.run("run_process_remote(list_ref,dict_ref)")
result=ray.get(process_large_lists_and_dicts.remote(list_ref,dict_ref))
print(result)
ray.shutdown()
